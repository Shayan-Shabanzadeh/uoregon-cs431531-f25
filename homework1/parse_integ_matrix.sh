#!/bin/bash
outdir="output"
outfile="results_integ_atomic.csv"
echo "method,variant,threads,steps,mean_time,mean_abs_err" > $outfile

for f in $outdir/integ_atomic_matrix_*.out; do
    ...
    grep -n "BEGIN BLOCK" "$f" | while read -r line; do
        ...
        times=$(echo "$block" | grep "Time to calculate Pi in //" | grep "steps" | awk '{print $NF}')
        [ -z "$times" ] && continue
        ...
        echo "$method,$variant,$threads,$steps,$mean_time,$mean_err" >> $outfile
    done
done
