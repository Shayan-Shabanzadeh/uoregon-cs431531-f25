...
void convert_csr_to_ell(unsigned int* csr_row_ptr, unsigned int* csr_col_ind,
                        ...
                        int* n_new)
{
    unsigned int max_per_row = 0;
    for (int i = 0; i < m; i++) {
        unsigned int row_begin = csr_row_ptr[i];
        unsigned int row_end   = csr_row_ptr[i + 1];
        unsigned int row_nnz   = row_end - row_begin;
        if (row_nnz > max_per_row) {
            max_per_row = row_nnz;
        }
    }

    *n_new = (int) max_per_row;

    unsigned int total = (unsigned int)m * max_per_row;

    unsigned int* ell_ci = (unsigned int*) malloc(sizeof(unsigned int) * total);
    double*       ell_v  = (double*)       malloc(sizeof(double) * total);
    assert(ell_ci);
    assert(ell_v);
    ...
    for (int row = 0; row < m; row++) {
        unsigned int row_begin = csr_row_ptr[row];
        unsigned int row_end   = csr_row_ptr[row + 1];
        unsigned int row_nnz   = row_end - row_begin;

        for (unsigned int k = 0; k < row_nnz; k++) {
            unsigned int src  = row_begin + k;
            unsigned int dest = k * m + row;

            ell_ci[dest] = csr_col_ind[src];
            ell_v[dest]  = csr_vals[src];
        }
    }
