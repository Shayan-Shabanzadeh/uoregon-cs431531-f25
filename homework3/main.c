...

{
    ...
    unsigned int *row_ptr = (unsigned int*) malloc((m + 1) * sizeof(unsigned int));
    unsigned int *col_out = (unsigned int*) malloc(nnz * sizeof(unsigned int));
    double       *val_out = (double*)        malloc(nnz * sizeof(double));
              ...
              omp_lock_t* writelock)
{
    ...
    for (int k = 0; k < nnz; k++) {
        ...
        double contrib = vals[k] * vector_x[c];
          ...
          double* vector_x, double *res)
{
    ...
    for (int i = 0; i < m; i++) {
        double sum = 0.0;
        for (unsigned int k = csr_row_ptr[i]; k < csr_row_ptr[i + 1]; k++) {
            unsigned int c = csr_col_ind[k];
            sum += csr_vals[k] * vector_x[c];
        }
        res[i] = sum;
    }
}
